...
ADMM_params = {'std_threshold':1e-6, 'gamma_val':1.0, 'gamma_factor':0.99, \
          ...
          'rel_tol':1e-5, 'stop_threshold':1e-9, 'max_iters':3e3, 'projection_lp': 2, 'x0':x0}


#optimize
best_sol,x_sol_ADMM,x_sol_AAADMM,x_sol_AADR,y1,y2,time_elapsed = ADMM_bqp_unconstrained(A/2, b, ADMM_params)
...
#show result
plt.figure()
image = x_sol_ADMM.reshape(I.shape, order='F')
plt.imsave('segmentation_result_ADMM.png', image, format="png", cmap='gray')

image = x_sol_AAADMM.reshape(I.shape, order='F')
plt.imsave('segmentation_result_AAADMM.png', image, format="png", cmap='gray')

image = x_sol_AAADMM.reshape(I.shape, order='F')
plt.imsave('segmentation_result_AADR.png', image, format="png", cmap='gray')
