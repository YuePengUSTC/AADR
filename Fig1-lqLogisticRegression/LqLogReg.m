function [x, intercp, FLAG, iter, betas, obj, err, eachtimes, combineres, Lagrangian, timing, err_rel, err_abs] = LqLogReg(A, b, q, mu, beta, offset)
  ...
  for k = 1:MAXITER 
    ...
    % w update
    w = w + beta * (x - z);
    % calculate combine residual
    r = (norm(x-z).^2 + norm(x-prev_x).^2);
    % record the values
    obj(k)     = Lq(z(2:end))*m*mu + fx;
     err(k)     = InfNorm(x - z);
    betas(k)   = beta;
    ...
    err_abs(k) = ABSTOL;
    eachtimes(k) = toc;
    combineres(k) = sqrt(r*beta/length(z));
    
    if VERBOSE 
      fprintf('%4d\t%.2f\t%10.4f\t%10.4g\n', k, eachtimes(k), obj(k), combineres(k));
    end
    % beta update
